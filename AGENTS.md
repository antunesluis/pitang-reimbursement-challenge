...
packages/backend/               — Express 5 + Prisma 7 + SQLite (libsql)
  ...
  src/lib/env.vars.ts           —   Zod-validated env vars (PORT, DATABASE_URL, JWT_SECRET, NODE_ENV)
  src/lib/errors.ts             —   AppError class ({ message, statusCode, errors? })
  src/lib/http-status.ts        —   HTTP status constants used by AppError
  src/lib/upload.ts             —   multer disk-storage config
  src/controllers/              —   route handlers (auth, user, category, reimbursement, attachment)
  src/routes/                   —   route wiring
  src/schemas/                  —   Zod validation schemas (body, params, query)
  src/middlewares/              —   auth JWT, role guard, validation, error fallback
  src/policies/                 —   permission checks (reimbursement.policy.ts)
  prisma/schema.prisma          —   data model
  prisma.config.ts              —   Prisma 7 config (datasource URL from env)
  prisma/seed.ts                —   creates 4 users + 3 categories + 1 sample DRAFT
  tests/setup.ts                —   cleanupDatabase() + seedAdmin() + helpers
packages/frontend/              — Vite 8 + React 19 + TanStack Router + Shadcn UI + Tailwind v4
  src/main.tsx                  —   entrypoint
  src/routes/                   —   file-based routes (__root.tsx, _authenticated/...)
  src/routeTree.gen.ts          —   auto-generated by TanStack Router (do not edit manually)
  src/components/               —   domain-split: auth/, categories/, dashboard/, layout/, reimbursements/, shared/, ui/, users/
  src/contexts/                 —   AuthContext (JWT cookie validation via /me)
  src/hooks/                    —   use-permissions, use-breadcrumb, use-mobile
  src/services/                 —   API service functions per domain
  src/schemas/                  —   Zod schemas (mirrors backend naming: amount, description, etc.)
  src/lib/api.ts                —   Fetch wrapper (Bearer token from cookies, 401 redirect)
  src/types/index.ts            —   Role/Status/Action const arrays + shared types (no enums)
DESAFIO.md                      — full project spec in Portuguese (requirements, entities, rules)
postman/                        — Postman collection covering all endpoints
```
...
- Prettier: 4-space indent (`useTabs: false`), single quotes, trailing commas, 80char width. Prettier-plugin-tailwindcss is active (sorts class names).
...
- 68 tests across 5 files: `tests/{auth,users,categories,reimbursements,attachments}.test.ts`.
...
Lockfile is `bun.lock` (JSON format, tracked in git).

## Postman

Collection at `postman/pitang-reimbursement-challenge.postman_collection.json` with 33 requests covering all endpoints. Token and IDs are auto-populated via test scripts when running logins and the full cycle.
