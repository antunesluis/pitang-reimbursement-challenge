...
function NewReimbursementPage() {
  ...
  async function onSubmit(data: CreateReimbursementFormData) {
    try {
      ...
      if (pendingFiles.length > 0) {
        await Promise.all(
          pendingFiles.map((file) => attachmentService.create(reimbursement.id, file)),
        );
      }
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="space-y-2">
              ...
              <CategorySelect onChange={(value) => setValue("categoryId", value)} value="" />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Attachments ({pendingFiles.length})</CardTitle>
        </CardHeader>
