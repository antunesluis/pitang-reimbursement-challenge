...
function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [reimbs] = await Promise.all([reimbursementService.list()]);
  ...
  const submittedCount = reimbursements.filter((r) => r.status === "SUBMITTED").length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {role === "EMPLOYEE" && (
          <>
            <StatsCard icon={Receipt} label="Total" value={reimbursements.length} />
            <StatsCard
        ...
        {role === "MANAGER" && (
          <>
            <StatsCard icon={Receipt} label="Total Pending" value={reimbursements.length} />
            <StatsCard
              className="border-blue-200 dark:border-blue-800"
              icon={Clock}
              label="Submitted"
            ...
            <StatsCard
              ...
              value={approvedCount}
            />
            <StatsCard icon={DollarSign} label="Paid" value={paidCount} />
          </>
        )}
        ...
        {role === "FINANCE" && (
          <>
            ...
            <StatsCard
              ...
              value={paidCount}
            />
            <StatsCard icon={Clock} label="Volume" value={reimbursements.length} />
          </>
        )}

        {role === "ADMIN" && (
          <>
            <StatsCard icon={Receipt} label="Reimbursements" value={reimbursements.length} />
            <StatsCard icon={Users} label="Users" value={userCount} />
            <StatsCard icon={Tags} label="Categories" value={categoryCount} />
            <StatsCard
